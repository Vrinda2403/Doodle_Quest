...
function PaperDrawing() {
  ...
  // Cloudinary Config
  const CLOUD_NAME = "dhzybzur5"; 
  const UPLOAD_PRESET = "dmxrlonj";
  ...
  const capture = async () => {
    ...
    try {
      ...
      if (!apiKey) {
        alert("Missing API Key! Make sure VITE_GROQ_API_KEY is in your .env file and restart the server.");
        setLoading(false);
        return;
      }
      ...
      // 1. Capture image frame to canvas
        ...
        messages: [
          {
            ...
            content: [
              {
                type: "text",
                text: "Identify the main object in this drawing. Return ONLY ONE WORD label (e.g. 'cat', 'tree')."
              },
              {
                type: "image_url",
                image_url: {
      ...
      const label = groqRes.choices[0]?.message?.content || "Unknown";
      console.log("Groq Prediction:", label);
      
      //  FIXED: Added backticks
      alert(`I think you drew a: ${label}`);
    ...
    <div className="min-h-screen bg-[#E8FFE8]">
      ...
      <div className="px-8 py-8 bg-[url(/src/assets/paperbg.png)] min-h-screen bg-cover bg-left-top">
        <div className="flex gap-8">
          ...
          {/* Left Side: Camera */}
          <div className="flex-1">
            <h1 className="text-3xl font-['Orbitron'] mb-3 text-gray-800">CAMERA FEED</h1>

            <div className="bg-white h-96 w-full max-w-[800px] border-2 border-black rounded-lg mb-4 flex items-center justify-center overflow-hidden">
              {/* Video Element */}
              <video
                ref={videoRef}
                autoPlay
            ...
            <div className="flex gap-4">
              ...
              <button
                className={`px-6 py-2 rounded-full text-white font-semibold transition ${
                  !cameraOn || loading ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"
                }`}
                onClick={capture}
                disabled={!cameraOn || loading}
              >
                {loading ? "Analyzing..." : "Capture & Guess"}
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 relative hidden lg:block">
            <img
              src="/src/assets/FamilyTable.png"
              alt="Family"
              className="absolute top-24 right-12 w-3/4 max-w-md"
            />
          </div>
        </div>
      </div>
